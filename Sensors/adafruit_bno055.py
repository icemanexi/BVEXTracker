...
class BNO055:  # pylint: disable=too-many-public-methods
    ...
    **Quickstart: Importing and using the device**
    ...
    def __init__(self) -> None:
        ...
        self.accel_range = ACCEL_2G
        self.gyro_range = GYRO_250_DPS
    ...
    def accel_bandwidth(self, bandwidth: int = ACCEL_1000HZ) -> None:
        if self.mode in [0x08, 0x09, 0x0A, 0x0B, 0x0C]:
...
class BNO055_I2C(BNO055):
    ...
    _acceleration = _ScaledReadOnlyStruct(0x08, "<hhh", np.float16(0.01))
    ...
    def check(self):
        mode = self._read_register(_GYRO_CONFIG_1_REGISTER)
        return bin(mode)
